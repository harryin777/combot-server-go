...
func (p *Provider) parseResponse(ctx context.Context, data []byte) (map[string]interface{}, error) {
	if len(data) < 4 {
	...
	if len(data) > 8 && data[8] == '{' {
		payload = data[8:]
		// log.Infof(ctx,"[DEBUG] payload偏移修正为data[8:]，首字节=%d", payload[0])
	} else {
	...
	if messageTypeSpecificFlags&0x02 != 0 {
		...
		//log.Infof(ctx,"收到最后一个包, PayloadSequence=%d", asr_result.PayloadSequence)
	}
	...
	case serverFullResponse:
		...
		if len(payload) > 0 && payload[0] == '{' {
			// log.Infof(ctx,"[DEBUG] 进入JSON直解析分支，payload长度=%d", len(payload))
			payloadMsg = payload
			payloadSize = int32(len(payload))
		} else {
			// log.Infof(ctx,"[DEBUG] 进入协议头解析分支，payload长度=%d", len(payload))
	...
	if payloadMsg != nil {
		...
		if serializationMethod == jsonFormat {
			...
			log.Debugf(ctx, "[DEBUG] parseResponse: JSON解析成功, 数据=%v", jsonData)
	...
	if len(data) > 0 && p.isStreaming {
		...
		if err := p.sendAudioData(ctx, data, false); err != nil {
			...
			if p.sendDataCnt%20 == 0 {
				log.Debugf(ctx, "发送音频数据成功, 长度: %d 字节", len(data))
			}
		}
	}
...
func (p *Provider) StartStreaming(ctx context.Context) error {
	...
	p.isStreaming = true
	log.Debugf(ctx, "[DEBUG] 流式识别初始化成功, connectID=%s, reqID=%s", p.connectID, p.reqID)
...
func (p *Provider) ReadMessage(ctx context.Context) {
	log.Infof(ctx, "[ASR] [doubao] 流式识别协程已启动")
	defer func() {
		if r := recover(); r != nil {
		...
		p.connMutex.Unlock()
		log.Infof(ctx, "[ASR] [doubao] 流式识别协程已结束")
	}()
	...
	for {
		...
		if !p.isStreaming || p.conn == nil {
			p.connMutex.Unlock()
			log.Infof(ctx, "流式识别已结束或连接已关闭，退出读取循环")
			return
		}
		conn := p.conn
		p.connMutex.Unlock()
		...
		if err != nil {
			p.setErrorAndStop(ctx, err)
			return
		}

		result, err := p.parseResponse(ctx, response)
		if err != nil {
			p.setErrorAndStop(ctx, fmt.Errorf("解析响应失败: %v", err))
			return
		}

		if code, hasCode := result["code"]; hasCode {
			log.Infof(ctx, "检测到code字段: 解析结果=%v", result)
			codeValue := code.(uint32)
			if codeValue != 0 {
				p.setErrorAndStop(ctx, fmt.Errorf("ASR服务端错误: Code=%d", codeValue))
				return
			}
		}
			...
			if resultData, hasResult := payloadMsg["result"].(map[string]interface{}); hasResult {
				...
				if listener := p.BaseProvider.GetListener(); listener != nil {
					if text == "" && p.SilenceTime() > idleTimeout {
						...
						log.Infof(ctx, "检测到静音超时, SilenceTime=%v/%v", p.SilenceTime(), idleTimeout)
						p.ResetStartListenTime()
					} else if text != "" {
					...
					if finished := listener.OnAsrResult(ctx, text, isLastPackage); finished {
						return
					}
				}
			} else if errorData, hasError := payloadMsg["error"]; hasError {
				// 处理错误响应中的 error 字段
				p.setErrorAndStop(ctx, fmt.Errorf("ASR响应错误: %v", errorData))
				return
			}
		}
...
func (p *Provider) setErrorAndStop(ctx context.Context, err error) {
	p.connMutex.Lock()
	defer p.connMutex.Unlock()
	p.err = err
	p.isStreaming = false
	...
	if strings.Contains(errMsg, "use of closed network connection") {
		log.Debugf(ctx, "setErrorAndStop: %v, sendDataCnt=%d", err, p.sendDataCnt)
	} else {
		log.Errorf(ctx, "setErrorAndStop: %v, sendDataCnt=%d", err, p.sendDataCnt)
	}
...
func (p *Provider) closeConnection(ctx context.Context) {
	defer func() {
		if r := recover(); r != nil {
			// 静默处理panic，避免程序崩溃
			log.Errorf(ctx, "关闭连接时发生错误: %v", r)
		}
	}()
...
func (p *Provider) SendLastAudio(ctx context.Context, data []byte) error {
	return p.sendAudioData(ctx, data, true)
}
