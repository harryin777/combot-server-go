...
type ConversationSession struct {
	...
	Status       ConversationStatus `json:"status" gorm:"column:status;type:tinyint(1);default:'active';comment:会话状态（active/closed）"`
...
type ConversationMessage struct {
	...
	Role        MessageRole    `json:"role" gorm:"column:role;type:tinyint(1);not null;comment:消息角色（user/assistant/system）"`
	Content     string         `json:"content" gorm:"column:content;type:text;not null;comment:消息内容"`
	MessageType MessageType    `json:"message_type" gorm:"column:message_type;type:tinyint(1);default:'text';comment:消息类型（text/image/audio）"`
